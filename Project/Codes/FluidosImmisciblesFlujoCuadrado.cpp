...
enum TipoCelda{fluidor,fluidob,ventilador,obstaculo};
...
void LatticeBoltzmann::ConstruyaLaGeometria(void){
  ...
  for(ix=1;ix<Lx;ix++)
    for(iy=0;iy<Ly;iy++){
	  ...
	  if((40<iy)&&(iy<80))
	Celda[ix][iy]=fluidor;
            else  Celda[ix][iy]=fluidob;
    }
	else  Celda[ix][iy]=fluidob;
	}
  for(ix=0,iy=0;iy<Ly;iy++)Celda[ix][iy]=ventilador; 
       }
void LatticeBoltzmann::Inicie(void){
  ...
  for(ix=0;ix<Lx;ix++)
	...
	else if(Celda[ix][iy]==fluidob){
	   fb[ix][iy][i]=fbnew[ix][iy][i]=feq(RhoBInicial,0,0,i,true);
	   fr[ix][iy][i]=frnew[ix][iy][i]=0;
	}
	else {
	   fr[ix][iy][i]=frnew[ix][iy][i]=0;
	     fb[ix][iy][i]=fbnew[ix][iy][i]=feq(RhoBInicial,UX0,UY0,i,true);
	}
}
}
...
void LatticeBoltzmann::Colisione1(int t){
  ...
  for(ix=0;ix<Lx;ix++) {//Para Cada Celda
	...
	if(ix=0){
	  frnew[ix][iy][i]=feq(Rho0,UX0,UY0,i,true);}
	else{
  ...
  for(ix=0;ix<Lx;ix++){ //Para Cada Celda
    for(iy=0;iy<Ly;iy++){
      ...
      for(i=0;i<9;i++){ //en cada direccion
	if(ix=0){
	  fbnew[ix][iy][i]=feq(Rho0,UX0,UY0,i,true);}
	else{
